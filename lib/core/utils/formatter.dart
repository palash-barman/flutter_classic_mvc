...
class Formatter {
  static String timeFormatter({
    ...
    if (showDate && dateTime != null) {
      ...
      if (dateOnly == yesterday) {
        result += "Yesterday at ";
      } else if (dateOnly == DateTime(now.year, now.month, now.day)) {
        result += "Today at ";
      } else {
        result += "${dateTime.day} ${monthName(dateTime.month)} at ";
      }
    }
  ...
  static String dateFormatter(DateTime date, {String format = "yyyy-MM-dd"}) =>
  ...
  /// TEXT
  static String toPascalCase(String text) => text
      .split(' ')
      .map(
        (w) => w.isEmpty
            ? w
  ...
  static String phoneFormatter(String phone, {String countryCode = "+1"}) {
    ...
    if (!phone.startsWith(countryCode.replaceAll('+', ''))) {
      phone = countryCode.replaceAll('+', '') + phone;
    }
    return "+$phone";
  }
